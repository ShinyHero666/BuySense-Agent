...
class DecisionEngineTest {
    ...
    DecisionEngineTest() throws Exception {
        engine = new DecisionEngine(new CatalogRepository(new ObjectMapper().findAndRegisterModules()));
    }
    ...
    void excludesSingleProductsAboveTheUserBudget() {
        ...
        assertThat(result.slate().get(0).product().id())
                .isEqualTo("phone-xiaomi-15");
    }
    ...
    void validatesOneHundredTwentyCategoryAndAdPolicyScenarios() {
        Map<String, List<String>> categoryPhrases = Map.of(
                ...
                "charger", List.of("充电器", "充电头", "充电线", "快充充电器", "charger"),
                ...
                for (String useCase : List.of("游戏", "办公")) {
                    for (boolean rejectAds : List.of(false, true)) {
                        String query = "预算10000元，推荐一款" + phrase + "，主要用于" + useCase
                                + (rejectAds ? "，不要广告" : "");
                        var result = engine.decide(parser.parse(query));
