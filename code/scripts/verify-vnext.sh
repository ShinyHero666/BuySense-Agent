...
echo "[1/8] Repository secret and build-context hygiene"
bash "$CODE_ROOT/scripts/check-repository-safety.sh"
"$PYTHON" "$CODE_ROOT/scripts/shopify_readonly_canary.py" --check

echo "[2/8] Shared contract drift"
"$PYTHON" "$CODE_ROOT/packages/contracts/generate_contracts.py" --check

echo "[3/8] Python unit and integration tests"
"$PYTHON" -m unittest discover -s "$CODE_ROOT/tests" -v

echo "[4/8] TypeScript/Pi control-plane gates"
...
echo "[5/8] Reproducible scale data and retrieval quality"
...
echo "[6/8] React/ECharts console"
...
echo "[7/8] Linux scripts"
bash -n "$CODE_ROOT"/scripts/*.sh
bash "$CODE_ROOT/scripts/test-run-sar-agent-config.sh"

echo "[8/8] Generated Python compile"
