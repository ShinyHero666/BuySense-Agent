...
export function QualityView(props: {
  ...
  const sourcesUnavailable = props.runtime !== null && props.runtime.dataPlane.retailSources === null;
  const sourcesUnavailableLabel = props.runtime?.dataPlane.mode === "memory"
    ? "内嵌数据面 / 未提供来源遥测"
    : "数据面不可用 / 未提供来源遥测";
  ...
  return (
    <div className="page quality-page">
      ...
      <div className="quality-grid">
        ...
        <section className="retail-sources-panel"><div className="section-title"><span>D</span><div><small>RETAIL DATA SOURCES</small><h2>零售数据来源</h2></div></div>
          <p className="source-intro">这里显示本次服务实际读取的数据，而不是仅显示启动配置。HTTP 失败后的本地替代会明确标为降级。</p>
          {sourcesUnavailable && <p className="source-unavailable">{sourcesUnavailableLabel}</p>}
          <div className="retail-source-grid">
            {(Object.keys(RETAIL_SOURCE_LABELS) as Array<keyof typeof RETAIL_SOURCE_LABELS>).map((key) => {
              const source = props.runtime?.dataPlane.retailSources?.[key];
              return (
                <article className="retail-source-card" data-testid={'retail-source-' + key} key={key}>
                  <header>
                    <span className={'signal ' + (source?.status === "up" ? "up" : source?.status === "down" ? "down" : "")} />
                    <small>{RETAIL_SOURCE_LABELS[key]}</small>
                    <b>{source ? SOURCE_STATUS_LABELS[source.status] : sourcesUnavailable ? "未提供" : "读取中"}</b>
                  </header>
                  <strong>{source ? EFFECTIVE_SOURCE_LABELS[source.effectiveSource] : sourcesUnavailable ? "未提供来源遥测" : "—"}</strong>
                  <div className="source-tags">
                    <span>{source ? CONFIGURED_MODE_LABELS[source.configuredMode] : "—"}</span>
                    <span title={source?.version ?? undefined}>版本 {source?.version ?? "—"}</span>
                  </div>
                  <small title={source?.effectiveProviderId ?? undefined}>实际 Provider：{source?.effectiveProviderId ?? "—"}</small>
                  {source?.configuredMode === "http" && <small title={source.providerId ?? undefined}>配置指纹：{source.providerId ?? "—"}</small>}
                  {source && <small className="source-telemetry">请求 {source.telemetry.requests} · 错误 {source.telemetry.errors} · 降级 {source.telemetry.fallbacks}</small>}
                  {source?.fallbackActive && <mark>已降级到本地快照</mark>}
                  {source?.lastErrorCode && <code>错误码：{source.lastErrorCode}</code>}
                </article>
              );
            })}
          </div>
